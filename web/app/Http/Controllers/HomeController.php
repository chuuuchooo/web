...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $totalRecords = FamilyPlanning::count() + ChildProfile::count();
        $familyPlanningCount = FamilyPlanning::count();
        $immunizationCount = ChildProfile::count();
    ...
    public function getEmployeeStatistics()
    {
        ...
        $activeEmployees = User::where('last_login_at', '>=', Carbon::now()->subDays(30))->count();

        // Get most active user (user with most records created)
        $mostActiveUser = User::select('users.*')
            ->selectRaw('(SELECT COUNT(*) FROM family_plannings WHERE created_by_user_id = users.id) + 
                        (SELECT COUNT(*) FROM child_profiles WHERE created_by_user_id = users.id) as total_records')
            ->orderBy('total_records', 'desc')
                ->first();
                
        // Calculate health worker activity percentage
        $totalRecords = FamilyPlanning::count() + ChildProfile::count();
        $healthWorkerActivity = $totalRecords > 0 
